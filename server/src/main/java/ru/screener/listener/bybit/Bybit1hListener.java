...
@Component
@Slf4j
public class Bybit1hListener extends AbstractKafkaListener<RsiDto> {

    private final StrategyProcessor<RsiDto> strategyProcessor;
    private final CaffeineDeduplicationService deduplicationService;

    public Bybit1hListener(StrategyProcessor<RsiDto> strategyProcessor,
    ...
    @KafkaListener(topics = "${app.kafka.topic.bybit1h.name}",
