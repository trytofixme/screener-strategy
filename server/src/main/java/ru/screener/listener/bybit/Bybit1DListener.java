...
@Component
@Slf4j
public class Bybit1DListener extends AbstractKafkaListener<RsiDto> {

    private final StrategyProcessor<RsiDto> strategyProcessor;
    private final CaffeineDeduplicationService deduplicationService;

    public Bybit1DListener(StrategyProcessor<RsiDto> strategyProcessor,
    ...
    @KafkaListener(topics = "${app.kafka.topic.bybit1D.name}",
